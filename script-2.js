...
<body>
  <main class="card">
    ...
    <p>The real page couldn't be reached from your browser, so Source × Sage is showing a small
...
button:hover { transform: translateY(-2px); }`,
  ...
  /* ============================================================
     EXTRACTION ENGINE
     ============================================================ */
  const OWN_WORKER_URL = "https://extract-cors.hiraarty505.workers.dev/?url="; // your Cloudflare Worker — tried first, before the public proxies
  const PROXIES = [
  ...
  async function populateViewer(url, result, usedDemo, durationMs) {
    ...
    viewerUrl.textContent = new URL(url).hostname;
    const demoBadgeEl = $("#demoBadge");
    if (demoBadgeEl) demoBadgeEl.hidden = !usedDemo;
